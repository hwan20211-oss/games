  ...
  return (
    <div className="mover-wrapper" style={{ maxWidth: '1000px' }}>
      ...
      <div className="game-container" style={{ gap: '1.5rem', alignItems: 'stretch' }}>
        {/* Left: Webcam (65%) */}
        <div className="video-card" ref={stageRef} style={{ flex: 1.8, height: 'auto', border: '8px solid #fff' }}>
          {!isLoaded && <div className="game-over-panel" style={{ background: '#fdfcfb' }}><div className="loading-circle" style={{ width: '50px', height: '50px', border: '4px solid #e2e8f0', borderTopColor: '#7B61FF', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div></div>}
          ...
          {isSuccessEffect && <div className="success-overlay" style={{ fontSize: '4rem', color: '#1e293b', textShadow: '0 0 20px rgba(0,0,0,0.05)' }}>🎉 SUCCESS!</div>}
        </div>

        {/* Right: Game Panel (35%) */}
        <div className="game-panel" style={{ flex: 1, background: '#fff', borderRadius: '32px', padding: '1.5rem', border: '1px solid #e2e8f0', boxShadow: '0 10px 30px rgba(0,0,0,0.02)' }}>
          <div className="tabs-header" style={{ marginBottom: '1.5rem', borderBottom: 'none', background: '#f8fafc', padding: '0.4rem' }}>
            <button className={`tab-btn ${activeTab === 'basic' ? 'active' : ''}`} onClick={() => setActiveTab('basic')} style={{ fontSize: '0.85rem', padding: '0.6rem 1.2rem' }}>🎯 기본</button>
            <button className={`tab-btn ${activeTab === 'custom' ? 'active' : ''}`} onClick={() => setActiveTab('custom')} style={{ fontSize: '0.85rem', padding: '0.6rem 1.2rem', opacity: 0.3 }}>✨ 커스텀</button>
          </div>

          <div className="battle-area" style={{ background: 'transparent', padding: 0 }}>
            {gameStatus === 'idle' && (
              <div style={{ textAlign: 'center', padding: '1rem' }}>
                <p style={{ fontSize: '0.9rem', marginBottom: '1.5rem', letterSpacing: 'normal', color: '#64748b', fontWeight: 500, lineHeight: 1.6 }}>
                  30초 동안 랜덤하게 나타나는<br/>제스처 미션을 수행하세요!
                </p>
            ...
            {gameStatus === 'playing' && (
              <div style={{ width: '100%', display: 'flex', flexKey: 'column', gap: '1.5rem', flexDirection: 'column' }}>
                <div style={{ textAlign: 'center' }}>
                  <span style={{ color: '#94a3b8', fontSize: '0.75rem', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '1px' }}>현재 미션</span>
                  <div style={{ fontSize: '4.5rem', margin: '0.5rem 0' }}>{GESTURES[mission]?.emoji}</div>
                  <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#1e293b' }}>{GESTURES[mission]?.label}를 유지하세요!</div>
                </div>

                <div style={{ width: '100%' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.4rem', fontSize: '0.8rem', color: '#64748b', fontWeight: 600 }}>
                    <span>유지 게이지</span>
                  ...
                  <div className="progress-track" style={{ height: '8px', background: '#f1f5f9' }}>
                    <div className="progress-fill" style={{ 
                      ...
                      boxShadow: holdProgress > 80 ? '0 0 10px rgba(0, 175, 255, 0.3)' : 'none'
                    }} />
                  </div>
                </div>

                <div className="panel-card" style={{ padding: '1.2rem', display: 'flex', justifyContent: 'space-around', alignItems: 'center', background: '#f8fafc', border: 'none' }}>
                   <div style={{ textAlign: 'center' }}>
                      <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 700 }}>SCORE</span>
                      <div style={{ fontSize: '2rem', fontWeight: 900, color: '#1e293b' }}>{score}</div>
                   </div>
                   <div style={{ width: '1px', background: '#e2e8f0', height: '30px' }}></div>
                   <div style={{ textAlign: 'center' }}>
                      <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 700 }}>TIME</span>
                      <div style={{ fontSize: '2rem', fontWeight: 900, color: timeLeft <= 10 ? '#f43f5e' : '#1e293b' }}>{timeLeft}s</div>
                   </div>
                </div>
              </div>
            )}
            ...
            {gameStatus === 'ended' && (
              <div style={{ textAlign: 'center', padding: '1rem' }}>
                <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>최종 점수</span>
                <span style={{ fontSize: '5rem', fontWeight: 900, color: '#1e293b', display: 'block', marginBottom: '1.5rem' }}>{score}</span>
      ...
      {debugData.length > 0 && debugData.map((hand, idx) => (
        <div key={idx} style={{ width: '100%', marginTop: '1.5rem', padding: '1.2rem', background: '#1e293b', borderRadius: '24px', fontFamily: 'monospace', fontSize: '11px', color: '#4ECDC4', overflowX: 'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
          <table style={{ width: '100%', tableLayout: 'fixed', borderCollapse: 'collapse' }}>
            <thead><tr style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.1)' }}><th style={{ textAlign: 'left', paddingBottom: '0.5rem' }}>Finger</th><th>P1</th><th>P2</th><th>Comp</th><th style={{ textAlign: 'right' }}>State</th></tr></thead>
            <tbody style={{ opacity: 0.9 }}>{hand.fingers.map((f, i) => (<tr key={i}><td style={{ padding: '0.2rem 0' }}>{f.name}</td><td>{f.c1}</td><td>{f.c2}</td><td style={{ color: '#64748b' }}>{f.comp}</td><td style={{ textAlign: 'right', color: f.res==='펴짐'?'#4ECDC4':'#f43f5e' }}>{f.res}</td></tr>))}</tbody>
          </table>
          <div style={{ marginTop: '12px', paddingTop: '10px', borderTop: '1px solid rgba(255, 255, 255, 0.1)', display: 'flex', justifyContent: 'space-between', fontWeight: 700 }}>
            <span>{hand.side} | OPEN: {hand.openCount}/5</span>
            <span style={{ color: '#fbbf24' }}>MATCH: {hand.gesture.emoji} {hand.gesture.label} ({hand.stability.count}/{hand.stability.total}f)</span>
          </div>
        </div>
      ))}
