...
export default function ObjectMover() {
  ...
  return (
    <div className="mover-wrapper">
      ...
      <div className="video-card" ref={stageRef} style={{ width: '100%', maxWidth: '640px', border: '8px solid #fff' }}>
        {!isLoaded && (
          <div className="game-over-panel" style={{ background: '#fdfcfb' }}>
             <div className="spinner-wave" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                <div className="loading-circle" style={{ width: '50px', height: '50px', border: '4px solid #e2e8f0', borderTopColor: '#4ECDC4', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
                <p style={{ color: '#4ECDC4', fontWeight: 800, letterSpacing: '1px' }}>AI ENGINE LOADING...</p>
             </div>
          </div>
        )}
        ...
        {isLoaded && gameStatus === 'playing' && (
          <>
            ...
            <div className="game-hud" style={{ top: '20px', left: '20px', right: '20px', position: 'absolute', display: 'flex', justifyContent: 'space-between', zIndex: 20 }}>
              <div className="mission-card" style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(8px)', border: '2px solid #4ECDC4', padding: '0.8rem 1.2rem', borderRadius: '16px', color: '#1e293b', fontWeight: 800 }}>
                {mission.emoji} {mission.label} GO!
              </div>
              <div className="score-display" style={{ fontSize: '3.5rem', color: '#1e293b', fontWeight: 900, textShadow: '0 0 20px rgba(0,0,0,0.05)' }}>{score}</div>
            </div>
            <div 
        ...
        {gameStatus === 'ended' && (
          <div className="game-over-panel" style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(10px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <span style={{ color: '#64748b', fontSize: '1.2rem', fontWeight: 700, marginBottom: '0.5rem' }}>TOTAL SCORE</span>
            <span style={{ fontSize: '6rem', fontWeight: 900, color: '#1e293b', marginBottom: '2rem' }}>{score}</span>
            <button className="btn-game-start" onClick={startGame} style={{ maxWidth: '200px' }}>RETRY</button>
          </div>
        )}

        {isLoaded && gameStatus === 'idle' && (
          <div className="game-over-panel" style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.4)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <button className="btn-game-start" onClick={startGame} style={{ maxWidth: '250px' }}>START MISSION</button>
          </div>
        )}
